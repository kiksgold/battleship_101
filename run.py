...
def start_game():
    ...
    start_game = input("Please enter your name:\n")
    grid_instruction = (
        "Please enter grid size between "
        + str(GRID_SIZE_MIN)
        + " and "
        + str(GRID_SIZE_MAX)
        + ":\n"
    ...
    ship_instruction = (
        "Please enter number of ship(s) between 1 and " + str(MAX_NUM_OF_SHIP) + ":\n"
    )
    ...
    # To create the size of the grid
    GRID_SIZE = grid_size

    # The number of ships a player can shoot
    ...
    def create_ships(grid):
        for ship in range(NUM_OF_SHIPS):
            ...
            while grid[ship_row][ship_column] == "X":
                ship_row, ship_column = randint(0, GRID_SIZE - 1), randint(
                    0, GRID_SIZE - 1
                )
